...
// Sadiqji is a specific named user, identified by the `is_sadiqji`
// flag on the profile row (set via SQL). Founders also pass.
export function canEditWeeklyPlanDates(user) {
  if (!user) return false
  return user.role === 'founder' || user.is_sadiqji === true
}

export function isSadiqji(user) {
  return !!user?.is_sadiqji
}
...
export async function getWeeklyPlanEntries(planId) {
  const { data, error } = await supabase
...
export async function upsertWeeklyPlanEntry(row) {
...
export async function deleteWeeklyPlanEntry(id) {
  const { error } = await supabase.from('weekly_plan_entries').delete().eq('id', id)
  if (error) throw error
}

// Uploads a File to spec-images/weekly-plan-<planId>/<entryKey>-<fabricKey>-<ts>.jpg.
// Reuses the existing `spec-images` bucket + compressImage pipeline.
export async function uploadWeeklyPlanImage(file, planId, entryKey, fabricKey) {
  if (!file) return null
  const compressed = await compressImage(file)
  const path = `weekly-plan-${planId || 'unsaved'}/${entryKey || 'entry'}-${fabricKey || 'fabric'}-${Date.now()}-${Math.random().toString(36).slice(2,7)}.jpg`
