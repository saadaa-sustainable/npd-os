...
// Uploads a File to spec-images/<styleKey>/<side>-<timestamp>.<ext>
// Returns the public URL.
export async function uploadSpecImage(file, styleKey, side) {
  if (!file) return null
  const ext = (file.name.split('.').pop() || 'jpg').toLowerCase()
  const path = `${styleKey || 'unsaved'}/${side}-${Date.now()}.${ext}`
