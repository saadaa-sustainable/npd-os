...
// Uploads a File to spec-images/weekly-plan-<planId>/fabric-<fabricKey>-<ts>.jpg.
// Reuses the existing `spec-images` bucket + compressImage pipeline.
export async function uploadWeeklyPlanImage(file, planId, fabricKey) {
  if (!file) return null
  const compressed = await compressImage(file)
  const path = `weekly-plan-${planId || 'unsaved'}/fabric-${fabricKey || 'new'}-${Date.now()}-${Math.random().toString(36).slice(2,7)}.jpg`
