...
//
// Style codes are 6 letters, no separators:
//   S + <gender:1> + <fabric:1> + <silhouette:1> + <suffix:AA-ZZ>
//   Example: SDCSAA  →  Saadaa + Women(D) + Cotton(C) + Shirt(S) + 1st in bucket
//
...
export const STYLE_CODE_SEGMENTS = ['gender','fabric','silhouette']
export const STYLE_CODE_PREFIX   = 'S'
export const STYLE_CODE_LENGTH   = 6
...
export function composeStyleCodeSegments(selections, rules) {
  ...
  for (const seg of STYLE_CODE_SEGMENTS) {
    const value = (selections[seg] || '').trim()
    if (!value) { prefix += '?'; missing.push(seg); continue }
    const match = (rules[seg] || []).find(r => r.value.toLowerCase() === value.toLowerCase())
    if (match) prefix += match.code
    else       { prefix += '?'; missing.push(seg) }
  }
